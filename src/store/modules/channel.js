...
export default {
  ...
  mutations: {
    ...
    updatePost (state, credentials) {
      state.posts.posts[state.posts.posts.indexOf(credentials.post)].message = credentials.post.message
    }
  },
  actions: {
    ...
    updatePosts ({commit}, credentials) {
      console.log(credentials)
      api.put('/channels/' + credentials.idChannel + '/posts/' + credentials.post._id, {message: credentials.post.message}).then(response => {
        console.log(response)
        commit('updatePost', credentials)
      })
    }
  }
}
